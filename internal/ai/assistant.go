...
func (a *Assistant) Categorize(ctx context.Context, items []string) ([]string, error) {
	...
	system := "You classify emails into exactly one category each, using these definitions:\n" +
		...
		// The three rules below were added after measuring them against 170
		// labelled emails (bench/, A/B over five prompt variants). On
		// granite-4.1-8b they moved raw accuracy 80.0% -> 88.2% and, weighted by
		// a real inbox mix (~87% Notification), 63.4% -> 81.8%.
		//
		// Output shape: without it a small model answers a one-email request with
		// a bare "[]" instead of [""] or ["Notification"] — 13 of 170 emails,
		// concentrated in the obvious ones (a dev newsletter, a firmware notice, a
		// resignation letter), each stored as no tag. With it: 0.
		" OUTPUT SHAPE, follow exactly: your reply is a JSON array with exactly as many elements as the input " +
		"array -- never fewer. An empty array [] is NEVER a valid reply. When no category applies to an email, " +
		"that email's element is the empty string \"\", so one email with no category is [\"\"] and never []. For one " +
		"email the reply is one element, e.g. [\"Notification\"] or [\"\"]." +
		// Notification precedence: the definition above says Notification loses to
		// Security and Newsletter but never that it BEATS Receipt/Finance/Calendar/
		// Needs reply, so automated mail leaked into those ("installation
		// finished" -> Receipt, "spend alert" -> Finance, "X requested time off" ->
		// Needs reply). Notification is ~87% of a real inbox, so its recall is the
		// only one that moves experienced quality: 60.0% -> 80.0%.
		" Precedence for automated mail: a message generated by a service about the recipient's own account, " +
		"servers, projects, files or usage is \"Notification\" even when it reports a COMPLETED action (an install, " +
		"a backup, a power cycle, a firmware update -- these are NOT \"Receipt\", which requires a purchase, " +
		"payment or physical delivery), even when it mentions spend, quota or usage (NOT \"Finance\", which " +
		"requires money owed or an account balance), even when its subject contains a word like meeting or sync, " +
		"and even when it names a person who did something (NOT \"Needs reply\" -- no human is waiting on an answer " +
		"from the recipient)." +
		// Acknowledgements: needed to counterbalance the rule above. Alone, the
		// precedence rule pushes the model to tag everything and no-tag recall
		// collapses to 0%; alone, this rule fixes no-tag but does nothing for
		// Notification. Together: Notification 80.0% AND no-tag 41.7%, with the
		// best "Needs reply" precision of any variant tried (77.8%).
		" \"Needs reply\" requires an OPEN question or request still waiting on the recipient. It is NOT a thank-you " +
		"or acknowledgement (\"Thanks!\", \"Received\", \"Much appreciated\"), NOT a closing pleasantry that ends a " +
		"thread, NOT a congratulation, NOT an informational FYI the sender says needs no action, and NOT a " +
		"message the recipient sent themselves. Those get \"\"."
	...
	zero := 0.0
	// Thinking stays off here (the Options default): classification is an argmax,
	// not a deliberation, and a thinking model is not merely slow — below a large
	// token budget it answers with nothing at all, so the inbox silently stops
	// being tagged.
	ch, err := a.stream(ctx, system, []Msg{{Role: RoleUser, Content: string(payload)}},
		Options{Temperature: &zero})
	if err != nil {
	...
	if strings.TrimSpace(b.String()) == "" {
		// A reasoning model whose endpoint ignored the thinking-off knobs spends its
		// whole budget in the reasoning channel and returns empty content. Say so,
		// because the symptom otherwise reads as "AI is broken" rather than
		// "this model needs thinking turned off".
		err := fmt.Errorf("model %s returned no content: if it is a reasoning model, "+
			"this endpoint may not honour reasoning_effort/enable_thinking", a.ActiveModel())
		logging.Trace("ai: categorize empty reply", "op", "Categorize", "err", err)
		return nil, err
	}
