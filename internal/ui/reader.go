...
func gistCard(gmailID, gist string) string {
	...
	return `<div class="mbgist" data-mid="` + html.EscapeString(gmailID) + `"` + hidden +
		` title="Written by AI from this message"><span class="mbgist-tag">✦ In short</span> <span class="mbgist-text">` +
		markdownToHTML(g) + `</span></div>`
}
...
func (w *window) fireGistJS(gmailID, gist string) {
	...
	// Rendered here, not in the page: the markup is built from escaped text by
	// markdownToHTML, so what crosses over is ours rather than the model's.
	gistJSON, _ := json.Marshal(markdownToHTML(gist))
...
.mbgist-tag{color:#1a5fb4;font-weight:600;font-size:78%;text-transform:uppercase;letter-spacing:.07em;margin-right:6px;white-space:nowrap}`
	...
	script := `<script nonce="` + nonce + `">(function(){var wrap,fitFrame=0;function fit(){var b=document.body;if(!b||!wrap)return;` +
		...
		// __mbGist fills a message's hidden gist placeholder in place (no content
		// swap, so the scroll position survives). What rides in is markup the app
		// built from escaped text (markdownToHTML), never the model's own string,
		// so innerHTML has nothing to inject. Placeholders are matched by dataset
		// comparison, not a selector, so a message id never needs CSS escaping.
		`window.__mbGist=function(id,text){if(!wrap)return;wrap.querySelectorAll('.mbgist').forEach(function(el){` +
		`if(el.dataset.mid!==id)return;var t=el.querySelector('.mbgist-text');if(t){t.innerHTML=text;}el.hidden=false;});requestFit();};` +
